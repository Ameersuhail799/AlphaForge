"""Risk Overlay Persistence Test Research Module.

Tests an N=3 consecutive-day persistence filter on the Risk Overlay strategy
for RELIANCE.NS and TCS.NS under realistic 2026 NSE delivery costs.
"""
...
def run_persistence_overlay_measurement(output_dir: Path | str = Path("reports") / "validation") -> Dict[str, Any]:
    """Execute N=3 Persistence-Filtered Risk Overlay audit for RELIANCE and TCS."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    logger.info("Building datasets for persistence overlay targets: %s", TARGET_ASSETS)
    ...
    def _run_overlay(asset: str, n_persist: int = 1, initial_cap: float = 100000.0) -> Dict[str, Any]:
    ...
    for asset in TARGET_ASSETS:
        res_filt_3 = _run_overlay(asset, n_persist=3)
        res_plain_overlay = _run_overlay(asset, n_persist=1)
        res_bh = _run_single_buy_hold(asset)
        res_champ = _run_single_champion(asset)

        transitions_removed = res_plain_overlay["transitions"] - res_filt_3["transitions"]
        pct_removed = round((transitions_removed / res_plain_overlay["transitions"]) * 100.0, 1)

        results[asset] = {
            "n3_filtered_overlay": res_filt_3,
            "plain_risk_overlay": res_plain_overlay,
            "buy_and_hold": res_bh,
            "champion_10d": res_champ,
            "transitions_removed": transitions_removed,
            "pct_transitions_removed": pct_removed,
        }

        csv_rows.append({"asset": asset, "strategy": "N=3 Filtered Overlay", **res_filt_3})
        csv_rows.append({"asset": asset, "strategy": "Plain Risk Overlay", **res_plain_overlay})
        csv_rows.append({"asset": asset, "strategy": "Buy and Hold", **res_bh})
        csv_rows.append({"asset": asset, "strategy": "Champion 10D", **res_champ})

    df_csv = pd.DataFrame(csv_rows)
    df_csv.to_csv(output_dir / "risk_overlay_persistence_summary.csv", index=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="N=3 Persistence-Filtered Risk Overlay Test")
    parser.add_argument("--output-dir", default="reports/validation", help="Output directory")
    args = parser.parse_args()

    res = run_persistence_overlay_measurement(args.output_dir)
    print("\n" + "=" * 60)
    print("N=3 PERSISTENCE-FILTERED RISK OVERLAY SUMMARY")
    print("=" * 60)
    for asset, d in res["results"].items():
        print(f"\nAsset: {asset.upper()}")
        print(f"  N=3 Filtered Overlay: CAGR = {d['n3_filtered_overlay']['cagr_pct']:>6.2f}%, Sharpe = {d['n3_filtered_overlay']['sharpe']:>5.2f}, Sortino = {d['n3_filtered_overlay']['sortino']:>5.2f}, MaxDD = {d['n3_filtered_overlay']['max_dd_pct']:>6.2f}%, Transitions = {d['n3_filtered_overlay']['transitions']}")
        print(f"  Plain Risk Overlay  : CAGR = {d['plain_risk_overlay']['cagr_pct']:>6.2f}%, Sharpe = {d['plain_risk_overlay']['sharpe']:>5.2f}, Sortino = {d['plain_risk_overlay']['sortino']:>5.2f}, MaxDD = {d['plain_risk_overlay']['max_dd_pct']:>6.2f}%, Transitions = {d['plain_risk_overlay']['transitions']}")
        ...
        print(f"  Transitions Removed : {d['transitions_removed']} ({d['pct_transitions_removed']}%)")
